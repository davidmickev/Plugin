...
@PluginDescriptor(
        ...
        description = "Left click cook",
        tags = {"one", "click", "oneclick", "cook"}
)
...
public class OneClickFletchPlugin extends Plugin {
    ...
    private final List<Integer> banks = List.of(21301); // kourend cooking area

    private final int RAW = ItemID.RAW_KARAMBWAN;
    private final int COOKED = ItemID.COOKED_KARAMBWAN;

    private final int RANGE = 21302;
...
//    private final int CLOSE_BANK_ID = 786434;
    ...
    private void onClientTick(ClientTick event) {
        if (
            ...
            String text = "<col=00ff00>One Click Cook";
    ...
    private Boolean isCooking(MenuOptionClicked event) {
        return client.getLocalPlayer().getAnimation() == AnimationID.COOKING_RANGE;
    }

    private void cookLoop(MenuOptionClicked event) {
        logger("got event");
        if (isCooking(event)) {
            logger("we are currently cooking, no op");
            ...
            if (inBank()) {
                logger("in bank");
                if (isRawInventory(event)) {
                    logger("logs detected, close bank");
                    closeBank(event);
                }
                else if (!isCooking(event)){
                    logger("bows detected, deposit bows");
                    depositAll(event);
                }
                else {
                    logger("no fish of any kind detected, withdraw food");
                    withdrawFish(event);
                }
            }
            // if not in bank, start fletching or open bank to deposit
                ...
                logger("not in bank");
                if (isRawInventory(event) && !inMenu) {
                    logger("logs in inventory, start fletching");
                    useKnifeOnLog(event);
                }
                else if (!isCooking(event)) {
                    logger("cooked items, open bank");
                    openBank(event);
                }
                else if (!isRawInventory(event)) {
    ...
    private Boolean isRawInventory(MenuOptionClicked event) {
        ...
        if (inventory!=null && !inventory.isHidden() && inventory.getDynamicChildren()!=null) {
            ...
            return (int) inventoryItems.stream().filter(item -> item.getItemId() == RAW).count() >= 1;
        }
...
//    private Boolean isCookedInventory(MenuOptionClicked event) {
//        Widget inventory = client.getWidget(WidgetInfo.INVENTORY.getId());
//        Widget bankInventory = client.getWidget(WidgetInfo.BANK_INVENTORY_ITEMS_CONTAINER.getId());
//
//        if (inventory!=null && !inventory.isHidden() && inventory.getDynamicChildren()!=null) {
...
//    }
    ...
    private void useKnifeOnLog(MenuOptionClicked event) {
        // set menu entry to use knife on log
        Widget fish = getInventoryItem(RAW);

        //clay oven
        GameObject BANK = new GameObjectQuery().idEquals(21302).result(client).nearestTo(client.getLocalPlayer());

        if (BANK == null || fish == null) return;

        client.setSelectedSpellWidget(WidgetInfo.INVENTORY.getId());
        client.setSelectedSpellChildIndex(knife.getIndex());
        client.setSelectedSpellItemId(knife.getItemId());

        event.setMenuEntry(createMenuEntry(0, MenuAction.WIDGET_TARGET_ON_WIDGET, fish.getIndex(), 9764864, true));
        inMenu = true;
    }
    ...
    public void depositAll(MenuOptionClicked event) {
        event.setMenuEntry(createMenuEntry(1, MenuAction.CC_OP, 11, 786474, false)); //close bank
    }
    ...
    private void withdrawFish(MenuOptionClicked event) {
        event.setMenuEntry(createMenuEntry(
                1,
                MenuAction.CC_OP,
                getBankIndex(RAW),
